...
def load_CNN_model_newclassifier(weight_path, lr, in_shape, drop1, drop2):
    ...
    # add a new dense relu and softmax
    new_model = models.Sequential(layers=[model, layers.Dense(5, activation='relu'),
                                          layers.Dense(2, activation='softmax')])

    new_model.summary()
    new_model.compile(optimizer=Adam(learning_rate=lr), loss='categorical_crossentropy')
...
def load_CNN_model_unfreeze_1layer(weight_path, lr, drop, num_unfreeze):
    # this model needs to be of the same form as the pre-trained model
    model = models.Sequential()
...
def generate_inputs(train_dir, tns=None, n_epoch=100, batch_size=10, train_ratio=0.75, restrict_neg=True):
    ...
    # neg_path3 = base_dir + 'loop_training/neg3'
    pos_files, neg_files = [], []
...
if __name__ == '__main__':
    ...
    if method == "NEW_CLASSIFIER":
        model = load_CNN_model_newclassifier(weight_path, lr, in_shape, drop1, drop2)
    elif method == "UNFREEZE":
        if num_layer == 1:
            model = load_CNN_model_unfreeze_1layer(weight_path, lr, drop2, num_unfreeze)
        else:
            model = load_CNN_model_unfreeze(weight_path, lr, in_shape, drop1, drop2, num_unfreeze)
    else:
        print("Invalid method. Proceeding with unfreeze.")
